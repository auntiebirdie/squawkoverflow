...
(async () => {
  ...
  for (let siteMember of siteMembers) {
	  ...
	  promises.push(Database.query('UPDATE counters SET `count` = (SELECT COUNT(*) FROM birdypets WHERE member = ?) WHERE type = "aviary" AND member = ? AND id = "total"', [siteMember.id, siteMember.id]));
