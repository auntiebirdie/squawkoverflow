...
(async () => {
  ...
  let members = await Database.query('SELECT id FROM members WHERE id != "nGfzpk7VZQhGWfTBNy8XWa"');
  ...
  for (let member of members) {
    ...
    await Database.query('UPDATE member_settings SET `member` = ? WHERE `member` = ?', [id, member.id]);
    await Database.query('UPDATE member_variants SET `member` = ? WHERE `member` = ?', [id, member.id]);
