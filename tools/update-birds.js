    ...
    response.pipe(csv())
      .on('data', async (row) => {
        if (row['CATEGORY'] == 'species') {
          ...
          conn.query('INSERT INTO squawkdata.species VALUES (?, ?, ?, ?) ON DUPLICATE KEY UPDATE family = ?, commonName = ?, scientificName = ?', [row['SPECIES_CODE'], family, row['PRIMARY_COM_NAME'], row['SCI_NAME'], family, row['PRIMARY_COM_NAME'], row['SCI_NAME']]);
      ...
      .on('end', async () => {
        let specials = [
          ['redjun1', 'Phasianidae', 'Chicken', 'Gallus gallus domesticus', 'Phasianidae', 'Chicken', 'Gallus gallus domesticus']
        ];

        for (let special of specials) {
          conn.query('INSERT INTO squawkdata.species VALUES (?, ?, ?, ?) ON DUPLICATE KEY UPDATE family = ?, commonName = ?, scientificName = ?', special);
